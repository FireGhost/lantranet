...
<template>
  ...
  <UForm v-else>
    <UFieldGroup
      v-for="playerSubscription in animation.players"
      :key="playerSubscription.playerId"
    >
      <UBadge
        color="neutral"
        variant="outline"
        size="lg"
        :label="playerSubscription.player.username"
        class="w-60"
      />
      <UInput
