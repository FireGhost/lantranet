...
const { data: menuItems } = await useFetch<MenuItemModel[]>(
  "/api/buvette/menu-items",
);

function onMenuItemChange(menuItemId: number) {
  const menuItemUpdated = menuItems.value?.find(
  ...
  <div v-for="menuItem in menuItems" :key="menuItem.id">
    <UFieldGroup>
      ...
      <UBadge color="neutral" variant="outline">
        <USwitch
          :default-value="menuItem.isAvailable"
          @change="onMenuItemChange(menuItem.id)"
        />
      </UBadge>
    </UFieldGroup>
  </div>
